...
# Load the LSP client
spec_lsp = importlib.util.spec_from_file_location("terraform_lsp_client", "./terraform_lsp_client.py")
    ...
    try:
        ...
        if terraform_check.returncode == 0:
            ...
            results["terraform"] = {
                ...
                "version": version_check.stdout.strip() if version_check.returncode == 0 else "version check failed"
            }
        else:
            results["terraform"] = {"available": False, "error": "terraform not found"}
        
        # Check terraform-ls
        terraformls_check = subprocess.run(["which", "terraform-ls"], capture_output=True, text=True)
        if terraformls_check.returncode == 0:
            version_check = subprocess.run(["terraform-ls", "version"], capture_output=True, text=True)
            ...
            results["terraform_ls"] = {"available": False, "error": "terraform-ls not found"}
...
def terry_lsp_debug() -> Dict[str, object]:
    ...
    try:
        ...
        try:
            version_result = subprocess.run(["terraform-ls", "version"], capture_output=True, text=True, timeout=10)
            results["terraform_ls_binary"] = {
                ...
                "error": version_result.stderr.strip() if version_result.returncode != 0 else None
            }
        except subprocess.TimeoutExpired:
            results["terraform_ls_binary"] = {"available": False, "error": "timeout"}
        except FileNotFoundError:
            results["terraform_ls_binary"] = {"available": False, "error": "binary not found"}
        except Exception as e:
        ...
        if terraform_lsp_client._lsp_client:
            results["lsp_client"] = {
                ...
                "process_active": terraform_lsp_client._lsp_client.terraform_ls_process is not None
            }
        else:
        ...
        try:
            help_result = subprocess.run(["terraform-ls", "serve", "--help"], capture_output=True, text=True, timeout=5)
...
def terry_workspace_info(path: str = ".") -> Dict[str, object]:
    ...
    try:
        ...
        results["terraform_state"] = {
            ...
            "state_file_exists": os.path.exists(os.path.join(full_path, "terraform.tfstate"))
        }
        
        # Check for common Terraform files
        common_files = ["main.tf", "variables.tf", "outputs.tf", "providers.tf", "terraform.tf", "versions.tf"]
        ...
        if len(tf_files) == 0:
            results["lsp_readiness"]["recommended_actions"].append("Create Terraform files (.tf)")
...
async def terry_lsp_init(workspace_path: str) -> Dict[str, object]:
    ...
    try:
        ...
        if not os.path.exists(full_workspace_path):
            return {"terry-lsp-init": {"error": f"Workspace {full_workspace_path} does not exist"}}
...
@mcp.tool()
def terry_workspace_setup(path: str, project_name: str = "terraform-project") -> Dict[str, object]:
    """
...
def terraform_lsp_status() -> Dict[str, object]:
    ...
    if terraform_lsp_client._lsp_client and terraform_lsp_client._lsp_client.initialized:
        return {
            "terraform-ls-status": {
                "status": "active",
                "initialized": True,
