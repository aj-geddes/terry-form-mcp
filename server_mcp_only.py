...
# Initialize FastMCP
mcp = FastMCP(name="terry-form-mcp", version="3.0.0")
...
def validate_and_normalize_path(
    path: str, workspace_root: str = "/mnt/workspace"
) -> Optional[str]:
    """
...
try:
    ...
    def initialize_terraform_cloud():
        ...
        if api_token:
            ...
            logger.info(
                "Terraform Cloud API token not found, running without Cloud integration"
            )
...
async def terry(
    ...
    for action in actions:
        ...
        if action not in ALLOWED_TERRAFORM_ACTIONS:
            return {
                "error": f'Unknown action "{action}". Allowed: {", ".join(ALLOWED_TERRAFORM_ACTIONS)}'
            }
    ...
    if path.startswith("github://") and REPO_HANDLER:
        ...
        if len(parts) < 2:
            return {
                "error": "Invalid GitHub path format. Use: github://owner/repo/path"
            }
        ...
        workspace_result = await REPO_HANDLER.prepare_terraform_workspace(
            owner=owner, repo=repo, config_path=config_path
        )
...
if REPO_HANDLER:
    ...
    async def github_clone_repo(
        owner: str, repo: str, branch: str = None, force: bool = False
    ) -> dict:
        ...
        return await REPO_HANDLER.clone_or_update_repo(
            owner=owner, repo=repo, branch=branch, force=force
        )
    ...
    async def github_list_terraform_files(
        ...
        return await REPO_HANDLER.list_terraform_files(
            owner=owner, repo=repo, path=path, pattern=pattern
        )
    ...
    async def github_prepare_workspace(
        owner: str, repo: str, config_path: str, workspace_name: str = None
    ) -> dict:
...
async def terry_workspace_info(path: str = ".") -> dict:
    ...
    if target_path.exists() and target_path.is_dir():
        ...
        info["terraform_state"]["terraform_dir_exists"] = (
            target_path / ".terraform"
        ).exists()
        info["terraform_state"]["state_file_exists"] = (
            target_path / "terraform.tfstate"
        ).exists()
        ...
        if github_repos_dir.exists():
            info["github_repos"] = [
                d.name for d in github_repos_dir.iterdir() if d.is_dir()
            ]
...
async def main():
    ...
    logger.info(
        f"Module Intelligence: {'Enabled' if MODULE_INTELLIGENCE else 'Disabled'}"
    )
