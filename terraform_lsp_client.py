...
class TerraformLSPClient:
    ...
    async def start_terraform_ls(self, workspace_path: str) -> bool:
        ...
        try:
            ...
            if not os.path.exists(workspace_path):
                ...
                self.initialization_error = f"Workspace path does not exist: {workspace_path}"
                return False
            
            # Ensure terraform-ls binary exists
            result = subprocess.run(["which", "terraform-ls"], capture_output=True, text=True)
            ...
            version_result = subprocess.run(["terraform-ls", "version"], capture_output=True, text=True)
            if version_result.returncode == 0:
                self.logger.info(f"terraform-ls version: {version_result.stdout.strip()}")
            ...
            if self.terraform_ls_process.returncode is not None:
                ...
                self.logger.error(f"terraform-ls process exited immediately: {stderr.decode()}")
                self.initialization_error = f"terraform-ls process exited: {stderr.decode()}"
                return False
    ...
    async def validate_document(self, file_path: str) -> Dict:
        ...
        try:
            if not self.initialized:
                return {"error": "LSP client not initialized", "initialization_error": self.initialization_error}
    ...
    async def get_hover_info(self, file_path: str, line: int, character: int) -> Dict:
        ...
        try:
            ...
            return {"success": True, "hover": None, "message": "No hover information available"}
    ...
    if _lsp_client is None:
        ...
        if workspace_path and not _lsp_client.initialized:
            ...
            if not success:
                ...
                _lsp_client = None
                raise RuntimeError(f"Failed to initialize LSP client: {_lsp_client.initialization_error if _lsp_client else 'Unknown error'}")
