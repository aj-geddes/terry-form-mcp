...
class TestTerraformExecutor(TerryFormTestBase):
    ...
    def test_terraform_init(self, test_workspace, terraform_config):
        ...
        project_path = self.create_terraform_project(
            test_workspace / "init_test", terraform_config
        )

        # Run terraform init
        result = self.run_command("terraform init -input=false", cwd=str(project_path))
    ...
    def test_terraform_validate(self, test_workspace, terraform_config):
        ...
        project_path = self.create_terraform_project(
            test_workspace / "validate_test", terraform_config
        )
        ...
        # Run validate
        result = self.run_command("terraform validate", cwd=str(project_path))

        assert result[
            "success"
        ], f"Terraform validate failed: {result.get('stderr', '')}"
...
"""
        project_path = self.create_terraform_project(
            test_workspace / "fmt_test", unformatted
        )

        # Check formatting
        result = self.run_command("terraform fmt -check", cwd=str(project_path))
    ...
    def test_terraform_plan(self, test_workspace, terraform_config):
        ...
        # Create and init project
        project_path = self.create_terraform_project(
            test_workspace / "plan_test", terraform_config
        )
        ...
        result = self.run_command(
            "terraform plan -input=false -no-color", cwd=str(project_path)
        )
    ...
    def test_terraform_plan_with_vars(self, test_workspace, terraform_config):
        ...
        project_path = self.create_terraform_project(
            test_workspace / "plan_vars_test", terraform_config
        )
        ...
        assert result[
            "success"
        ], f"Terraform plan with vars failed: {result.get('stderr', '')}"
    ...
    def test_invalid_terraform_config(self, test_workspace, invalid_terraform_config):
        ...
        project_path = self.create_terraform_project(
            test_workspace / "invalid_test", invalid_terraform_config
        )
    ...
    def test_concurrent_executions(self, test_workspace, terraform_config):
        ...
        def run_terraform_in_dir(dir_name):
            project_path = self.create_terraform_project(
                test_workspace / dir_name, terraform_config
            )

            # Init and validate
            init_result = self.run_command(
                "terraform init -input=false", cwd=str(project_path)
            )
            ...
            validate_result = self.run_command(
                "terraform validate", cwd=str(project_path)
            )
